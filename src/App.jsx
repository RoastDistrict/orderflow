...
function OrderCard({order,onOpen,onDelete,onEdit,onReopen,dim=false}){
  ...
  return <div style={{position:"relative",marginBottom:10}}>
    ...
    <div
      style={{background:C.card,border:`1px solid ${ready&&!billed?C.greenBd:C.border}`,borderRadius:12,padding:"14px 16px",opacity:dim?0.65:1,boxShadow:"0 1px 3px rgba(0,0,0,0.05)",userSelect:"none"}}
...
// ─── PENDING SKU TAB ──────────────────────────────────────────
function PendingSkuTab({orders,onUpdate}){
  const [expandSku,setExpandSku]=useState(null);
  // Aggregate pending items across all live orders
  ...
  if(skus.length===0)return <div style={{textAlign:"center",color:C.textFaint,padding:"60px 0",fontSize:13}}>No pending items across live orders.</div>;

  return <>
    <div style={{fontSize:11,fontWeight:700,color:C.textDim,letterSpacing:"0.8px",marginBottom:12}}>PENDING SKUS — ACROSS ALL LIVE ORDERS</div>
    {skus.map(skuRow=>{
            ...
            return <div key={i} style={{padding:"12px 16px",borderBottom:i<skuRow.orders.length-1?`1px solid ${C.border}`:"none",display:"flex",alignItems:"center",justifyContent:"space-between",gap:8}}>
              ...
              {isPending
                ?<div style={{display:"flex",gap:6}}>
                    <Btn onClick={()=>onUpdate(occ.orderId,occ.sIdx,occ.iIdx,{status:"fulfilled",handledBy:"Staff"})} color="greenO" sx={{padding:"6px 10px",fontSize:11}}>✓ Done</Btn>
                    <Btn onClick={()=>onUpdate(occ.orderId,occ.sIdx,occ.iIdx,{status:"unavailable",handledBy:"Staff"})} color="redO" sx={{padding:"6px 10px",fontSize:11}}>✕ N/A</Btn>
                  </div>
...
// ─── STAFF HOME ───────────────────────────────────────────────
function StaffHome({orders,staffName,onNewOrder,onOpenOrder,onSignOut,onEditOrder,onReopenOrder,onItemUpdate}){
  ...
  return <div style={{minHeight:620,display:"flex",flexDirection:"column",background:C.bg}}>
    <div style={{padding:"16px 20px 12px",borderBottom:`1px solid ${C.border}`,background:"#fff"}}>
      ...
      <div style={{display:"flex",gap:4,background:C.bg,borderRadius:8,padding:3,border:`1px solid ${C.border}`}}>
        ...
        <button onClick={()=>setTab("pending")} style={tabSt(tab==="pending")}>📦 Pending SKUs</button>
    ...
    <div style={{flex:1,overflowY:"auto",padding:"14px 20px 40px"}}>
      {tab==="live"&&<>
        ...
        {inProg.length>0&&<><GrpLabel label="In Progress" color={C.amber}/>{inProg.map(o=><OrderCard key={o.id} order={o} onOpen={onOpenOrder} onEdit={onEditOrder} onReopen={onReopenOrder}/>)}</>}
        {ready.length>0&&<><GrpLabel label="Ready for Billing" color={C.green}/>{ready.map(o=><OrderCard key={o.id} order={o} onOpen={onOpenOrder} onEdit={onEditOrder} onReopen={onReopenOrder}/>)}</>}
        {live.length===0&&<div style={{textAlign:"center",color:C.textFaint,padding:"60px 0",fontSize:13}}>No orders yet today.</div>}
      </>}
      {tab==="pending"&&<PendingSkuTab orders={orders} onUpdate={onItemUpdate}/>}
      {tab==="history"&&<>
    ...
    <div style={{flex:1,overflowY:"auto",padding:"20px 20px 40px"}}>
      ...
      {stage==="analysing"&&<>
        ...
        <div style={{background:C.amberBg,border:`1px solid ${C.amberBd}`,borderRadius:10,padding:16,textAlign:"center"}}>
          ...
          <div style={{fontSize:12,color:C.textDim}}>{visionMode==="premium"?"Claude Vision is extracting text":"Google Vision is extracting text"}</div>
        </div>
      </>}
      {stage==="analyseError"&&<>
      ...
      {stage==="reviewing"&&ext&&<>
        ...
        {ext.sections.map((sec,sIdx)=><div key={sIdx} style={{marginBottom:20}}>
          <div style={{fontSize:11,fontWeight:700,letterSpacing:"0.8px",color:C.textDim,marginBottom:8,display:"flex",alignItems:"center",gap:8}}>
            ...
            <BuyerTypeahead
              ...
              style={{flex:1}}
            />
            {(sec._confidence!==undefined&&sec._confidence<95)||(!sec._confidence&&sec._raw)?<span style={{fontFamily:C.mono,fontSize:9,fontWeight:700,background:C.amberBg,border:`1px solid ${C.amberBd}`,borderRadius:4,padding:"2px 6px",color:C.amber,whiteSpace:"nowrap"}}>UNMATCHED</span>:null}
          </div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8,marginTop:8}}>
          <Btn onClick={confirm} color={canConfirm?"green":"ghost"} sx={{width:"100%",padding:13,fontSize:14,opacity:canConfirm?1:0.5,cursor:canConfirm?"pointer":"not-allowed"}}>{canConfirm?`✓ Create ${ext.sections.filter(s=>s.items.some(i=>!i.skipped&&i.sku)).length} Order${ext.sections.filter(s=>s.items.some(i=>!i.skipped&&i.sku)).length>1?"s":""}`:`Review ${lowConfPending.length} item${lowConfPending.length>1?"s":""} first`}</Btn>
...
function AdminApp({orders,users,skuList,catList,onSignOut,onOrderUpdate,onOrderBilled,onOrderReopen,onAddOrder,onUserChange,onDeleteOrder,onSkuChange,skuListEnriched,buyerList,onBuyerChange,buyerGroups,onBuyerGroupChange}){
  ...
  if(scanning)return <ScanScreen actorName="Admin" onBack={()=>setScanning(false)} onConfirm={o=>{onAddOrder(o);setScanning(false);}} skuList={eSkus} catList={catList}/>;
...
export default function App(){
  ...
  if(screen==="staff-scan")return <ScanScreen actorName={actorName} onBack={()=>setScreen("staff-app")} onConfirm={o=>{addOrder(o);setScreen("staff-app");}} skuList={enrichedSkuList} catList={catList} buyerList={buyerList}/>;
  if(screen==="staff-app"){
    ...
    return <StaffHome orders={orders} staffName={actorName} onSignOut={()=>{setStaffId(null);setScreen("choose");}} onNewOrder={()=>setScreen("staff-scan")} onOpenOrder={id=>setActiveOrderId(id)} onEditOrder={id=>{setActiveOrderId(id);}} onReopenOrder={id=>reopenOrder(id)} onItemUpdate={(orderId,sIdx,iIdx,changes)=>updateItem(orderId,sIdx,iIdx,changes)}/>;
  }
