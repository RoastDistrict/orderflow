        ...
        {(() => {
          ...
          const rows=handled.filter(i=>i.status!=="unavailable").map(item=>{
            ...
            return <React.Fragment key={item.id}>
              <span style={{fontFamily:C.mono,fontSize:12,color:C.text,padding:"8px 0",borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center"}}>{item.sku}{item.qty!==item.origQty&&<span style={{fontSize:10,color:C.textDim,marginLeft:4}}>(req ×{item.origQty})</span>}</span>
              <span style={{fontFamily:C.mono,fontSize:12,color:C.amber,fontWeight:700,padding:"8px 0",borderBottom:`1px solid ${C.border}`,textAlign:"right",display:"flex",alignItems:"center",justifyContent:"flex-end"}}>×{item.qty}</span>
              <span style={{fontFamily:C.mono,fontSize:12,color:C.textDim,padding:"8px 0",borderBottom:`1px solid ${C.border}`,textAlign:"right",display:"flex",alignItems:"center",justifyContent:"flex-end"}}>₹{rate>0?rate:"—"}</span>
              <span style={{fontFamily:C.mono,fontSize:12,color:rate>0?C.text:C.textFaint,fontWeight:rate>0?700:400,padding:"8px 0",borderBottom:`1px solid ${C.border}`,textAlign:"right",display:"flex",alignItems:"center",justifyContent:"flex-end"}}>{rate>0?`₹${amt.toLocaleString("en-IN")}`:"—"}</span>
            </React.Fragment>;
          });
    ...
    <div style={{flex:1,overflowY:"auto",padding:"16px 20px 40px"}}>
      ...
      {tab==="orders"&&(()=>{
        ...
        return <>
          ...
          {filteredOrders.map(o=><OrderCard key={o.id} order={o} onOpen={setActiveOId} onDelete={onDeleteOrder} dim={o.status==="billed"}/>)}
        </>;
      })()}
