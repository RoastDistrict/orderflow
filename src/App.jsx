...
const purgeOldOrders = (orders) => orders.filter((o) => o.status==="historical"||o.date >= SEVEN_DAYS_AGO);
...
function StaffHome({orders,staffName,onNewOrder,onOpenOrder,onSignOut,onEditOrder,onReopenOrder,onItemUpdate,lang,setLang,buyerList=[]}){
  ...
  const hist=orders.filter(o=>o.status==="billed"||o.status==="historical");
  ...
  return <div style={{minHeight:620,display:"flex",flexDirection:"column",background:C.bg}}>
    ...
    <div style={{flex:1,overflowY:"auto",padding:"14px 20px 40px"}}>
      {(tab==="live"||tab==="processed"||tab==="pending-order"||tab==="history")&&<input value={search} onChange={e=>setSearch(e.target.value)} placeholder={lang==="hi"?"नाम या SKU खोजें…":"Search by name or SKU…"} style={{width:"100%",padding:"9px 12px",borderRadius:8,border:`1px solid ${C.border}`,fontSize:13,fontFamily:C.sans,outline:"none",color:C.text,background:"#fff",boxSizing:"border-box",marginBottom:12}}/>}
      {tab==="live"&&<>
      ...
      {tab==="processed"&&<>
        {filterOrders(processed).length===0&&<div style={{textAlign:"center",color:C.textFaint,padding:"60px 0",fontSize:13}}>No processed orders.</div>}
      ...
      {tab==="pending-order"&&<>
        <div style={{background:C.redBg,border:`1px solid ${C.redBd}`,borderRadius:10,padding:"10px 14px",fontSize:12,color:C.red,marginBottom:12}}>These orders have unfulfilled items from previously billed orders.</div>
        {filterOrders(pendingOrders).length===0&&<div style={{textAlign:"center",color:C.textFaint,padding:"60px 0",fontSize:13}}>No pending orders.</div>}
      ...
      {tab==="history"&&<>
        ...
        {dates.map(d=><div key={d} style={{marginBottom:20}}><GrpLabel label={fmtDate(d)} color={C.textDim}/>{filterOrders(grouped[d]).map(o=><OrderCard key={o.id} order={o} onOpen={onOpenOrder} onEdit={onEditOrder} onReopen={onReopenOrder} dim lang={lang} buyerList={buyerList}/>)}</div>)}
      </>}
    </div>
  </div>;
}
    ...
    <div style={{padding:"14px 20px",borderBottom:`1px solid ${C.border}`,background:"#fff"}}>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:10}}>
        ...
        <div style={{flex:1}}>
          ...
          <div style={{fontSize:11,color:C.textDim}}>{order.scannedAt}{isBilled&&<span style={{color:C.blue,marginLeft:6}}>· Billed</span>}{isProcessed&&<span style={{color:C.indigo,marginLeft:6}}>· Processed</span>}</div>
        </div>
...
function AdminApp({orders,users,skuList,catList,onSignOut,onOrderUpdate,onOrderBilled,onOrderReopen,onOrderProcess,onOrderSendToBilling,onAddOrder,onUserChange,onDeleteOrder,onSkuChange,skuListEnriched,buyerList,onBuyerChange,buyerGroups,onBuyerGroupChange,onAddAlias}){
  ...
  // Orders tab filter
  const [orderFilter,setOrderFilter]=useState("all"); // all | live | processed | pending-order | billed
  ...
  // Analytics
  const unfulfilled=[];orders.filter(o=>o.status!=="historical").forEach(o=>o.sections.forEach(sec=>sec.items.forEach(item=>{if(item.status==="unavailable"||item.status==="partial")unfulfilled.push({sku:item.sku,customer:sec.name,orderId:o.id,date:o.date,type:item.status,reqQty:item.origQty,sentQty:item.qty||0});})));
  ...
  return <div style={{minHeight:620,display:"flex",flexDirection:"column",background:C.bg}}>
    ...
    <div style={{flex:1,overflowY:"auto",padding:"16px 20px 40px"}}>
      ...
      {tab==="orders"&&(()=>{
        ...
        const baseFiltered=orderFilter==="live"?liveOrders:orderFilter==="processed"?processedOrders:orderFilter==="pending-order"?pendingOrdersList:orderFilter==="billed"?billedOrders:allOrders;
        ...
        return <>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
            <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:6,flex:1}}>
              {[[allOrders.length,"All",C.text,"all"],[liveOrders.length,"Live",C.amber,"live"],[processedOrders.length,"Proc.",C.indigo,"processed"],[pendingOrdersList.length,"Pend.",C.red,"pending-order"],[billedOrders.length,"Billed",C.green,"billed"]].map(([v,l,c,f])=><div key={l} onClick={()=>setOrderFilter(f)} style={tileSt(orderFilter===f,c)}>
      ...
      {/* ── HISTORICAL TAB ── */}
      {tab==="historical"&&(()=>{
        const historical=orders.filter(o=>o.status==="historical");
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    // Midnight archive: move billed orders from previous days to "historical"
    const archiveToHistorical=()=>{
      const nowDate=new Date().toISOString().slice(0,10);
      setOrders(prev=>{
        prev.filter(o=>o.status==="billed"&&o.date<nowDate).forEach(o=>{
    ...
    archiveToHistorical();
    // Schedule next run at 23:59
    const scheduleNightlyArchive=()=>{
      const now=new Date(),target=new Date(now);
      target.setHours(23,59,0,0);
      if(target<=now)target.setDate(target.getDate()+1);
      const ms=target-now;
      return setTimeout(()=>{archiveToHistorical();scheduleNightlyArchive();},ms);
    };
    const nightlyTimer=scheduleNightlyArchive();
    return()=>{unsubOrders();unsubUsers();unsubSkus();unsubCats();unsubBuyers();unsubBuyerGroups();clearInterval(purgeInterval);clearTimeout(nightlyTimer);};
  },[]);
  ...
  // sendTobilling: splits order — fulfilled items billed, unfulfilled items become a new pending order
  const sendTobilling=orderId=>{
    ...
    const pendingSections=order.sections.map(sec=>({...sec,items:sec.items.filter(i=>i.status==="pending").map(i=>({...i,status:"pending",handledBy:null}))})).filter(s=>s.items.length>0);
    // Update current order to billed (fulfilled items only)
    ...
    // Create new pending order for unfulfilled items
