...
import hindiTokens from "./hindiTokens.json";
...
// Which orders belong in the live `orders` node. Cut-offs are recomputed on every call
// (2026-09-25) — they used to be frozen at page load, so an app left open never aged anything out.
const orderCutoffs = () => ({ yday: localDateISO(new Date(Date.now()-86400000)), week: localDateISO(new Date(Date.now()-7*86400000)) });
const keepLive = (o, c=orderCutoffs()) => {
  if(["live","processed","pending-order"].includes(o.status)) return true; // never move active orders
  return (o.date||"") >= c.week; // historical + billed: kept 7 days, then deleted (Archit, 2026-09-25)
};
const purgeOldOrders = (orders) => { const c=orderCutoffs(); return orders.filter(o=>keepLive(o,c)); };
// Delete aged-out orders from orders/ (Archit, 2026-09-25: historical orders are deleted after
// 7 days and not kept anywhere — no archive node, no backup copy; billing history is in Tally). One multi-path write per
// batch, working on the raw DB entries [[key, order], …] — NOT the filtered on-screen list.
// Batches of 200 keep each write small; the orders listener re-fires after each write and
// the next batch follows until the live node holds only active + recent orders.
// Rule (keepLive): active never deleted; historical and billed kept 7 days.
const CLEANUP_BATCH = 200;
const archiveOldOrders = (entries) => {
  const c=orderCutoffs();
  const old=(entries||[]).filter(([k,o])=>k&&o&&!keepLive(o,c)).slice(0,CLEANUP_BATCH);
  if(!old.length) return;
  const u={};
  old.forEach(([k])=>{ u[`orders/${k}`]=null; });
  update(ref(db),u).catch(e=>_dbErr("cleanup",e));
};
...
// ─── HINDI NAME TRANSLITERATION (runtime fallback) ────────────
// Registered buyers carry a curated hindiName (buyers/{id}.hindiName). For order
// or section names that match no ledger (one-off buyers), transliterate on the fly
// so Hindi mode never falls back to Latin. Uses the shipped token dictionary
// (hindiTokens.json — the same dict the buyer hindiNames were built from) plus a
// light phonetic fallback, so results stay consistent with the stored hindiNames.
const _hiVow=(s,i)=>{for(const v of["aa","ee","oo","ai","au","ou","ii"])if(s.substr(i,2)===v)return[v,2];return "aeiou".includes(s[i])?[s[i],1]:[null,0];};
const _hiRule=(w)=>{
  const core=String(w).toLowerCase().replace(/[^a-z]/g,"");
  if(!core)return w;
  const IND={a:"अ",aa:"आ",i:"इ",ee:"ई",ii:"ई",u:"उ",oo:"ऊ",e:"ए",o:"ओ",ai:"ऐ",au:"औ",ou:"औ"};
  const MAT={a:"",aa:"ा",i:"ि",ee:"ी",ii:"ी",u:"ु",oo:"ु",e:"े",o:"ो",ai:"ै",au:"ौ",ou:"ौ"};
  const DIG={sh:"श",ch:"च",th:"थ",ph:"फ",kh:"ख",gh:"घ",bh:"भ",dh:"ध",jh:"झ",ck:"क",ng:"ंग"};
  const CON={b:"ब",c:"क",d:"द",f:"फ",g:"ग",h:"ह",j:"ज",k:"क",l:"ल",m:"म",n:"न",p:"प",q:"क",r:"र",s:"स",t:"त",v:"व",w:"व",x:"क्स",y:"य",z:"ज़"};
  let out="",i=0,pend=false;
  while(i<core.length){
    const g=_hiVow(core,i),v=g[0],vl=g[1];
    if(v){out+=pend?MAT[v]:IND[v];i+=vl;pend=false;continue;}
    const two=core.substr(i,2);let ch;
    if(DIG[two]){ch=DIG[two];i+=2;}else{ch=CON[core[i]]||"";i+=1;}
    out+=ch;pend=!!ch;
  }
  return out;
};
const _hiGet=(k)=>Object.prototype.hasOwnProperty.call(hindiTokens,k)?hindiTokens[k]:undefined;
const _hiLETTER={a:"ए",b:"बी",c:"सी",d:"डी",e:"ई",f:"एफ",g:"जी",h:"एच",i:"आई",j:"जे",k:"के",l:"एल",m:"एम",n:"एन",o:"ओ",p:"पी",q:"क्यू",r:"आर",s:"एस",t:"टी",u:"यू",v:"वी",w:"डब्ल्यू",x:"एक्स",y:"वाय",z:"ज़ेड"};
const _hiKEEP=new Set(["gpm","vki","riico","gps","gpb","dcm","nsr","llp"]);
// Transliterate one alphanumeric run: dictionary → kept code → digits → single letter
// name → short all-caps acronym spelled out → phonetic fallback.
const _hiRun=(run)=>{
  const ps=run.toLowerCase(), hit=_hiGet(ps);
  if(hit!==undefined) return hit;
  if(_hiKEEP.has(ps)) return run.toUpperCase();
  if(/^[0-9]+$/.test(run)) return run;
  if(run.length===1&&/[a-z]/i.test(run)) return _hiLETTER[ps];
  if(/^[A-Z]+$/.test(run)&&run.length<=4) return run.split("").map(c=>_hiLETTER[c.toLowerCase()]).join("");
  return _hiRule(run);
};
// A whitespace-delimited token: whole-token dict hit first (keeps "j.k." → जे.के.),
// else split on internal punctuation into runs so "J.K.Enterprises" → जे.के.एंटरप्राइजेज.
const _hiTok=(w)=>{
  const s=w.toLowerCase(), hit=_hiGet(s);
  if(hit!==undefined) return hit;
  if(_hiKEEP.has(s)) return w.toUpperCase();
  return w.split(/([A-Za-z0-9]+)/).map(p=>/^[A-Za-z0-9]+$/.test(p)?_hiRun(p):p).join("");
};
const translitHi=(name)=>String(name||"").split(/(\s+)/).map(p=>/^\s+$/.test(p)?p:(p==="&"?"एंड":_hiTok(p))).join("");
// Buyer/section name for display in the current language: ledger hindiName if the
// name matches a buyer, else an on-the-fly transliteration (Hindi mode only).
const hiBuyer=(name,buyerList,lang)=>{
  if(lang!=="hi")return name;
  const b=(buyerList||[]).find(x=>x.name===name);
  return (b&&b.hindiName)?b.hindiName:translitHi(name);
};
...
function OrderCard({order,onOpen,onDelete,onEdit,onReopen,dim=false,lang="en",buyerList=[]}){
  ...
  // Hindi mode: ledger hindiName if matched, else transliterate the raw name on the fly
  const buyerObj=buyerList.find(b=>b.name===firstName);
  const displayName=hiBuyer(firstName,buyerList,lang);
  const names=order.sections.map(s=>hiBuyer(s.name,buyerList,lang)).join(", ");
          ...
          {row.orders.map((occ,i)=>{
            ...
            return <div key={i} style={{padding:"12px 16px",borderBottom:i<row.orders.length-1?`1px solid ${C.border}`:"none",display:"flex",alignItems:"center",justifyContent:"space-between",gap:8}}>
              <div
                ...
                <div style={{fontSize:13,fontWeight:600,color:C.text}}>{lang==="hi"?translitHi(occ.buyer):occ.buyer}{canMove&&isPending&&<span style={{fontSize:9,color:C.textFaint,marginLeft:6,fontWeight:400}}>⟶ hold to move</span>}</div>
                <div style={{fontFamily:C.mono,fontSize:11,color:C.textDim,marginTop:2}}>{mode==="cat"?occ.sku+" · ":""}{occ.orderId} · ×{occ.qty}{occ.orderRef&&occ.orderRef.date?<span style={{color:C.amber}}> · {fmtDate(occ.orderRef.date)}</span>:""}</div>
              </div>
              {isPending
    ...
    {menuFor&&<div onClick={e=>{if(e.target===e.currentTarget)setMenuFor(null);}} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.45)",zIndex:200,display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:"16px 16px 0 0",padding:"20px 20px 28px",width:"100%",maxWidth:480,boxSizing:"border-box"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6}}>
          <span style={{fontFamily:C.mono,fontWeight:700,fontSize:14,color:C.text}}>{lang==="hi"?translitHi(menuFor.buyer):menuFor.buyer}</span>
...
function OwnerApp({buyerList=[],catList=[],skuList=[],orders=[],partyRates={},onSetFolderRate,onSetSeriesPartyRate,onSetSeriesListRate,onBuyerChange,ownerPassword="6666",onOwnerPasswordChange,onSignOut}){
  const [bucket,setBucket]=useState("all"); // 2026-09-25: default to full ledger list (bucket picker removed)
  ...
  const back=()=>{if(folderId){setFolderId(null);setEdits({});}else if(buyerId){setBuyerId(null);setFolderId(null);setEdits({});}else onSignOut();};
  ...
  return <div style={{minHeight:620,display:"flex",flexDirection:"column",background:C.bg}}>
    {/* Client rates home — flat ledger list (bucket picker removed 2026-09-25) */}
    {bucket&&!buyer&&<>
      <div style={{padding:"18px 20px 14px",borderBottom:`1px solid ${C.border}`,background:"#fff",display:"flex",alignItems:"center",gap:12}}>
        <div style={{flex:1,minWidth:0}}>
          ...
          <div style={{fontSize:11,color:C.textDim,marginTop:2}}>{parties.length.toLocaleString("en-IN")} ledgers · tap to set rates</div>
        </div>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"14px 20px 26px"}}>
        ...
        {parties.length===0&&<EmptyState icon="📒" title="No ledgers yet"/>}
        <div style={{marginTop:20,background:"#fff",border:`1px solid ${C.indigoBd}`,borderRadius:12,padding:15}}>
...
export default function App(){
  ...
  const rawOrdersRef=useRef([]); // unfiltered orders/ snapshot for the archiver
  ...
  const [authReady,setAuthReady]=useState(false); // flips true once anonymous auth resolves; DB listeners wait for it
  ...
  // Wait for Firebase Anonymous Auth to resolve before attaching DB listeners.
  // Scoped rules require `auth != null`; without this gate the listeners fire
  // before sign-in completes → permission-denied → app hangs on the loading
  // screen (this is what forced the 2026-04-17 rules revert). Backward-compatible
  // with the current open rules (auth still resolves, just gates a few ms later).
  useEffect(()=>{
    const unsub=onAuthStateChanged(fbAuth,u=>{ if(u) setAuthReady(true); });
    return ()=>unsub();
  },[]);

  useEffect(()=>{
    if(!authReady) return; // don't attach listeners until anonymous auth has resolved
    // Orders listener
    const ordersRef=ref(db,"orders");
    let archiveTimer=null;
    const unsubOrders=onValue(ordersRef,snap=>{
      const val=snap.val();
      rawOrdersRef.current=val?Object.entries(val):[]; // unfiltered DB snapshot — what the archiver works on
      if(val){
        ...
        archiveTimer=setTimeout(()=>{
          ...
          archiveOldOrders(rawOrdersRef.current); // delete aged-out orders (2026-09-25)
        },2000);
      }
      else{setOrders([]);}
      setLoading(false);
    },err=>{console.error("[OrderFlow] orders read failed (check auth/rules):",err);setLoading(false);});
    // Users listener
    ...
    // Hourly cleanup (fixed 2026-09-25). The old version diffed the already-filtered on-screen
    // list against itself, so it never removed anything and every phone kept re-downloading
    // months of history. Now: re-hide with fresh cut-offs, and archive from the DB snapshot.
    const purgeInterval=setInterval(()=>{
      setOrders(prev=>purgeOldOrders(prev));
      archiveOldOrders(rawOrdersRef.current);
    },60*60*1000);
