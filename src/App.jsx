...
function OrderCard({order,onOpen,onDelete,onEdit,onReopen,dim=false}){
  ...
  // Derive 2-char initials from first buyer name
  const firstName=order.sections[0]?.name||"";
  const initials=(()=>{const words=firstName.trim().split(/\s+/).filter(Boolean);if(words.length===0)return"?";if(words.length===1)return words[0].slice(0,2).toUpperCase();return(words[0][0]+words[1][0]).toUpperCase();})();
...
function ScanScreen({actorName,onBack,onConfirm,skuList,catList,buyerList=[],onAddAlias,lang="en",setLang}){
  ...
  const confirm=()=>{
    ...
    validSections.forEach(sec=>{
      ...
      onConfirm({id:genId(),date:TODAY,scannedAt:nowTime(),status:"live",notes:srcSec?.notes||"",sections:[sec]});
    });
  };
...
// ─── ORDER DETAIL ─────────────────────────────────────────────
function OrderDetail({order,actorName,isAdmin,onBack,onUpdate,onBilled,onReopen,skuList,catList,lang="en",setLang}){
  const t=T[lang];
