...
// ─── FIREBASE ─────────────────────────────────────────────────
// ⚠ TESTING-V2 BRANCH ONLY — hardcoded to the orderflowv2-9ed95 TEST project so the
// preview works with ZERO Vercel env vars. DO NOT merge this block to main: production
// must use the orderflow-defa4 config. At go-live, swap this block for the prod config.
...
function OwnerApp({buyerList=[],catList=[],skuList=[],orders=[],partyRates={},onSetFolderRate,onSetSeriesPartyRate,onSetSeriesListRate,ownerPassword="6666",onOwnerPasswordChange,onSignOut}){
  ...
  return <div style={{minHeight:620,display:"flex",flexDirection:"column",background:C.bg}}>
    {!bucket&&<>{hdr("OWNER","Per-party folder rates · ledgers")}
      <div style={{flex:1,overflowY:"auto",padding:"16px 20px",display:"flex",flexDirection:"column",gap:10}}>
        ...
        <div style={{marginTop:8,background:"#fff",border:`1px solid ${C.indigoBd}`,borderRadius:12,padding:16}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12}}>
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:11,fontWeight:700,color:C.indigo,letterSpacing:"0.5px"}}>🔑 OWNER PASSWORD</div>
              <div style={{fontSize:11,color:C.textDim,marginTop:3}}>Used to enter the Owner area · separate from Admin</div>
            </div>
            {!pwEditing&&<button onClick={()=>{setPwVal(String(ownerPassword||""));setPwEditing(true);}} style={{padding:"6px 12px",borderRadius:8,border:`1px solid ${C.border}`,background:"#fff",color:C.textDim,cursor:"pointer",fontSize:12,fontFamily:C.sans,fontWeight:600,flexShrink:0}}>Change</button>}
          </div>
          {pwEditing&&<div style={{marginTop:12}}>
            <input type="text" inputMode="numeric" value={pwVal} onChange={e=>setPwVal(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&pwVal.trim()){onOwnerPasswordChange(pwVal.trim());setPwEditing(false);}}} placeholder="New owner password" style={{width:"100%",padding:"10px 12px",borderRadius:8,border:`1px solid ${C.borderMd}`,fontSize:14,fontFamily:C.mono,letterSpacing:2,outline:"none",color:C.text,background:"#fff",boxSizing:"border-box",marginBottom:8}}/>
            <div style={{display:"flex",gap:8}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"14px 20px 40px"}}>
        ...
        {catList.map(c=>{const fn=folders[c.id]||{};const nS=c.series?Object.keys(c.series).length:0;const sm=folderSummary(c,fn);const has=!!sm;
          return <button key={c.id} onClick={()=>{setFolderId(c.id);setEdits({});}} style={{width:"100%",textAlign:"left",display:"flex",alignItems:"center",gap:10,padding:"12px 14px",minHeight:48,marginBottom:6,borderRadius:10,border:`1px solid ${has?C.indigo:C.border}`,background:"#fff",cursor:"pointer",fontFamily:C.sans}}>
            <div style={{flex:1,minWidth:0}}><div style={{fontSize:14,fontWeight:600,color:C.text}}>{c.name}</div><div style={{fontSize:11,color:has?C.indigo:C.textDim,fontWeight:has?600:400}}>{sm?sm:(c.partyPriced?"priced at billing":`${nS} series`)}</div></div>
    ...
    {buyer&&folderId&&dCat&&<>{hdr(dCat.name,`${buyer.name} · ${dCat.partyPriced?"party-priced":dSeries.length+" series"}`)}
      <div style={{flex:1,overflowY:"auto",padding:"14px 20px 40px"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8,padding:"4px 0 14px",borderBottom:`1px solid ${C.border}`,marginBottom:14}}>
          ...
          <button onClick={()=>{if(allowPct)setEdits(p=>({...p,disc:{mode:dMode==="pct"?"flat":"pct",value:dVal}}));}} title={allowPct?"Toggle % / ₹":"Flat ₹ only"} style={{width:32,height:32,borderRadius:6,border:`1px solid ${C.borderMd}`,background:"#fff",cursor:allowPct?"pointer":"default",opacity:allowPct?1:0.55,fontWeight:700,color:C.indigo,fontSize:13}}>{dMode==="flat"?"₹":"%"}</button>
          <input type="text" inputMode="numeric" value={dVal} onChange={ev=>{const nv=ev.target.value.replace(/[^0-9.]/g,"");setEdits(p=>({...p,disc:{mode:dMode,value:nv}}));}} onKeyDown={ev=>{if(ev.key==="Enter")onSetFolderRate(buyer.id,folderId,dVal===""?null:{mode:dMode,value:dVal});}} placeholder="0" style={{width:54,fontFamily:C.mono,fontSize:13,textAlign:"right",padding:"7px",border:`1px solid ${C.border}`,borderRadius:6,outline:"none",color:C.text}}/>
          <button onClick={()=>onSetFolderRate(buyer.id,folderId,dVal===""?null:{mode:dMode,value:dVal})} style={{padding:"8px 16px",borderRadius:7,border:"none",background:C.indigo,color:"#fff",fontWeight:700,fontSize:12,cursor:"pointer",flexShrink:0}}>Save</button>
        </div>
        ...
        {dSeries.map(([sid,s])=>{const pn=dFnode.series?.[sid];const pe=edits["p:"+sid];const pmodeRaw=pe?pe.mode:(pn?.mode||"abs");const pmode=(!allowPct&&pmodeRaw==="pct")?"flat":pmodeRaw;const pval=pe?pe.value:(pn!=null?String(pn.value):"");
          ...
          const eff=net!=null?net:discNet;
          return <div key={sid} style={{padding:"10px 0",borderBottom:`1px solid ${C.border}`}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              ...
              <span style={{flex:1}}/>
              <button onClick={()=>setEdits(p=>({...p,["p:"+sid]:{mode:allowPct?(pmode==="abs"?"pct":pmode==="pct"?"flat":"abs"):(pmode==="abs"?"flat":"abs"),value:pval}}))} style={{minWidth:40,height:30,padding:"0 6px",borderRadius:6,border:`1px solid ${C.borderMd}`,background:"#fff",cursor:"pointer",fontWeight:700,color:C.indigo,fontSize:12}}>{pmode==="abs"?"₹=":pmode==="flat"?"₹-":"%"}</button>
            ...
            {eff!=null&&<div style={{fontSize:11,color:C.indigo,textAlign:"right",marginTop:3}}>party pays ₹{eff}{net==null&&discNet!=null?<span style={{color:C.textDim,fontWeight:400}}> · after folder disc</span>:null}</div>}
          </div>;})}
        <div style={{fontSize:10,color:C.textDim,marginTop:12,lineHeight:1.5}}><strong>list ₹</strong> edits the master rate (everyone). Party toggle: <strong>₹=</strong> fixed{allowPct?<> · <strong>%</strong> off</>:null} · <strong>₹-</strong> ₹ off. A series rate overrides the folder discount. {allowPct?"":"Flat ₹ only for this folder. "}Saved instantly for {buyer.name}.</div>
      </div></>}
  </div>;
}
